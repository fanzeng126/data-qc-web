...
CREATE TABLE `drug_import_task_detail` (
  ...
  -- 文件类型映射
  `file_type` varchar(20) NOT NULL COMMENT '文件类型:HOSPITAL,CATALOG,INBOUND,OUTBOUND,USAGE',
...
```sql
CREATE TABLE `drug_post_hospital` (
...
└── sql/                             # 数据库脚本
    ...
    │   ├── drug_post_hospital.sql
    │   ├── drug_post_catalog.sql
    │   ├── drug_post_inbound.sql
    │   ├── drug_post_outbound.sql
    │   ├── drug_post_usage.sql
    │   └── drug_qc_rule.sql
    └── data/                        # 初始化数据
