            ...
            <div class="table-header">
              <div class="table-info">
                <div class="table-icon-wrapper">
                  <el-icon
                    class="table-icon"
                    :color="getTableIconColor(table.status)"
                    :size="24"
                  >
                    <Document />
                  </el-icon>
              ...
              <div class="table-status">
                <el-tag
                  :type="getStatusTagType(table.status)"
                  size="small"
                  class="status-tag"
                >
                  {{ table.statusDisplay }}
...
const timelineEvents = computed(() => {
  ...
  progressData.tableProgress?.forEach(table => {
    ...
    if (table.endTime) {
      const type = table.status === 4 ? 'success' :
        table.status === 5 ? 'danger' : 'warning'
      const icon = table.status === 4 ? SuccessFilled :
        table.status === 5 ? CircleCloseFilled : WarningFilled
  ...
  if (progressData.estimatedEndTime && progressData.overallStatus >= 4) {
    const type = progressData.overallStatus === 4 ? 'success' :
      progressData.overallStatus === 5 ? 'danger' : 'warning'
    const icon = progressData.overallStatus === 4 ? SuccessFilled :
      progressData.overallStatus === 5 ? CircleCloseFilled : WarningFilled
